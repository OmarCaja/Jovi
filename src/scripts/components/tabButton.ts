const buttons: NodeListOf<HTMLElement> =
  document.querySelectorAll(".js-tab-button");
...
if (buttons.length == 1) {
  const tabButtonsSections: HTMLElement = document.getElementById(
    "tab-buttons-section"
  ) as HTMLElement;
...
function bindTab(button: HTMLElement): void {
  const displayedSection: HTMLElement = document.getElementById(
    displayedSectionId
  ) as HTMLElement;
